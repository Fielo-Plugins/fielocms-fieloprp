...
var cmsThemeCss = [
  {
    ...
    dest: 'resource-bundles/FieloPlugin_PrpPlugin.resource/styles'
 }
];
...
gulp.task('cmsCopyResourcesFiles', () => {
  ...
  .pipe(gulp.dest('resource-bundles/FieloPlugin_PrpPlugin.resource'))
  .pipe($.size({title: 'copy'}));

  // copy images
  return gulp.src(['resources/images/**'], { dot: true})
  .pipe(gulp.dest('resource-bundles/FieloPlugin_PrpPlugin.resource/images'))
  .pipe($.size({title: 'copy'}));
});

// Builds zip for using as static resource
gulp.task('cmsBuildZip', ['cmsCopyResourcesFiles'], () => {
  return gulp.src('resource-bundles/FieloPlugin_PrpPlugin.resource/**')
    .pipe(zip('FieloPlugin_PrpPlugin.zip'))
